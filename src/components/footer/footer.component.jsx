/**
 * Trabalho Realizado por Micael Eid (2019112744), Nuno Almeida (2019121655) e Renato Craveiro (2018011392)
 * No âmbitio da Unidade Curricular de Linguagens Script (1º Ano - 2º Semestre)
 * ISEC - Licenciatura em Engenharia Informática - Ano Letivo 2021/2022
 */

import React from "react";

/**
 * Footer - Contém os autores da aplicação.
 * @returns 
 */
function Footer() {
  return (
    <footer className="container-fluid text-center">
      <h6>Micael Eid - 2019112744 | Nuno Almeida - 2019121655 | Renato Craveiro - 2018011392</h6>
      <h6>ISEC | Licenciatura em Engenharia Informática | Linguagens Script | Ano Letivo 2021/2022</h6>
    </footer>
  );
}

export default Footer;
